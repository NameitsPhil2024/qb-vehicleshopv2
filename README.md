# qb-vehicleshopv2
This is just a Bug fix the Original Script is Linked below!

That ist not my Script!
Das ist nicht mein Script!

here is the Original: https://github.com/fazitanvir/qb-vehicleshop-1

1. Download via the Green Buttom at the Top
2. Go to [qb-core] in your ressources and delete qb-vehicleshop
3. Paste the qb-vehicleshopv2 in your qb-core file an rename it. Its very Important to call it "qb-vehicleshop" or maybe it dont work
4. Extrakt the Images.zip in the imgs file!
5. Install the qb-vehicleshop.sql
6. now go to qb-core/client/function.lua and search for QBCore.Functions.GetVehicleProperties and replace it! around line 431
```  
...
```
7. Go to **QBCore.Functions.SetVehicleProperties** in the qb-core/client/function.lua and replace the code!
```
...
```
8. Go to the TOP of the File an add this two files
   above the --Callouts and paste the Code
```  
function Trim(s)
    return (s:gsub("^%s*(.-)%s*$", "%1"))
end
...
QBCore.Functions.GetVehicleProperties = function(vehicle)
    if DoesEntityExist(vehicle) then
        local color1, color2, color3 = GetVehicleCustomSecondaryColour(vehicle)
        local pearlescentColor, wheelColor = GetVehicleExtraColours(vehicle)
        local extras = {}
        ...
        return {
            model         = GetEntityModel(vehicle),
            plate         = Trim(GetVehicleNumberPlateText(vehicle)),  -- Zeile 411
            plateIndex    = GetVehicleNumberPlateTextIndex(vehicle),
            bodyHealth    = Round(GetVehicleBodyHealth(vehicle), 1),
            engineHealth  = Round(GetVehicleEngineHealth(vehicle), 1),
            tankHealth    = Round(GetVehiclePetrolTankHealth(vehicle), 1),
